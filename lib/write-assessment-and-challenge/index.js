...
async function writeAssessmentAndChallenge(
  ...
  return Promise.all(
    challengeItems.map(async (challengeItem) => {
      ...
      debug("challengeItem", challengeItem)
      ...
      if (!challengeConfig) {
        ...
        if (!authorooJson.challenges) {
          authorooJson.challenges = []
        }

        authorooJson.challenges.splice(challengeItem.position - 1, 0, challengeConfig);

        debug("authorooJson", JSON.stringify(authorooJson, null, 4))
